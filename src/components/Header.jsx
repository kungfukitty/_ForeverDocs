...
export default function Header(){
  ...
  return (<header className="section py-4 sm:py-6">
    <div className="flex items-center justify-between">
      <Link to="/" className="flex items-center gap-3">
        <img src="/logo.svg" alt={cfg.siteName} className="w-10 h-10"/><div className="font-extrabold tracking-tight text-xl">{cfg.siteName}</div>
      </Link>
      <div className="hidden sm:block">{nav}</div>
      <div className="sm:hidden"><button className="btn btn-secondary" onClick={()=>setOpen(v=>!v)} aria-label="Toggle navigation"><Menu className="w-5 h-5"/></button></div>
    </div>
